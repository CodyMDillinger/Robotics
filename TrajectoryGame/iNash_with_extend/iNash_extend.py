...
def add_edge(pt_new, pt_tree, color_, size, pywindow):
    pt_new.add_parent(pt_tree)
    pt_tree.add_child(pt_new)
    print 'new point, parents:', pt_new.parents
    print 'pt connected to, children:', pt_tree.children
...
def near_vertices(vertex_new, current_vertex, k, vertices_near):
    current_vertex.k_near = k
    near_radius = min(Dimensions.gamma * sqrt(log(k) / k), Dimensions.eta)       # function of numPoints
    # print 'near_radius:', near_radius
    if dist(vertex_new, current_vertex) < near_radius and current_vertex.at_goal_set is False:
        'near vertex found'
        vertices_near.append(current_vertex)
    for i in range(len(current_vertex.children)):
        if current_vertex.children[i].k_near != k:
            vertices_near = near_vertices(vertex_new, current_vertex.children[i], k, vertices_near)
    return vertices_near
##############################################################################################################


def extend_graph(x_rand, robot_root, obstacles, goal_set, pywindow, color_, k):  # add vertex and edges to sets for robot i
    print 'extend graph called, k =', k
    x_nearest = nearest(x_rand, robot_root, robot_root, k)  # find nearest vertex in robot i's graph, starting with root
    print 'xnearest: (', x_nearest.x, x_nearest.y, ')'
...
def main():
    pywindow, obstacles = init_pywindow('iNash Trajectory, Without Inter-robot Collision Checking')                       # set up pygame window, dimensions and obstacles
    ...
    while k < k_:                                                   # main loop
        for i in range(num_robots):                                 # for all bots
            ...
            extend_graph(x_rand, start[i], obstacles, goal_set[i], pywindow, robo_colors[i], k)
