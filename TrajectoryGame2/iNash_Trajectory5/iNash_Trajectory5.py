...
def form_trees_and_paths():  # this is the high-level of the algorithm as described at the source linked above
    ...
    all_bots, active_bots, inactive_bots, paths, costs, paths_prev, goal_pts, path_num, new_paths, time_to_path, changed = init_arrays(num_robots)
    ...
    for i in range(len(goal_pts[0])):
        display_paths(goal_pts[0][i].paths, pywindow, Colors.dark_green)
    return pywindow, buttons, active_bots, paths, costs, robo_colors, k_end, time_to_path, time_waited, num_robots
##############################################################################################################


def main():
    pywindow, buttons, active_bots, paths, costs, robo_colors, k_end, robot_times, total_time, numbots = cProfile.run('form_trees_and_paths()')
    ...
    print 'robot times:', robot_times
    write_data_to_csv(robot_times)
