...
def nearest2(vertex_rand, current_vertex, prev_vertex, nearest_, axis_, tree_num):  # uses kd tree to efficiently return approximate nearest vertex
    # print 'optimized nearest, axis:', axis_.axis
    if norm(current_vertex, vertex_rand) < norm(nearest_, vertex_rand) and current_vertex.at_goal_set is False and current_vertex.tree_num == tree_num:
    ...
    if next_vert1 is not None:  # if there is a child in this direction
        nearst = nearest2(vertex_rand, next_vert1, current_vertex, nearst, axis_.next_, tree_num)
    if next_vert2 is not None:  # if there is a child in this direction
        if abs(getattr(current_vertex, axis_.axis) - getattr(vertex_rand, axis_.axis)) > norm(nearst, vertex_rand):
            ...
            nearst = nearest2(vertex_rand, next_vert2, current_vertex, nearst, axis_.next_, tree_num)
    return nearst
##############################################################################################################
...
def near_vertices2(vertex_new, current_vertex, prev_vertex, k, vertices_near, axis_, tree_num):           # uses kd tree to efficiently return approximate nearest vertex
    if k == 1:
        radius = Dimensions.eta
    else:
        radius = min(Dimensions.gamma * sqrt(log(k) / k), Dimensions.eta)       # function of numPoints
    if radius > norm(current_vertex, vertex_new) > 0 and current_vertex.at_goal_set is False and tree_num == current_vertex.tree_num:
    ...
    if getattr(vertex_new, axis_.axis) > getattr(current_vertex, axis_.axis):   # if new vertex is to the right of or above the current vertex
        ...
        next_vert2 = current_vertex.right_child
    if next_vert1 is not None:  # if there is a child in this direction
        vertices_near, radius = near_vertices2(vertex_new, next_vert1, current_vertex, k, vertices_near, axis_.next_, tree_num)
    if next_vert2 is not None:  # if there is a child in this direction
        if abs(getattr(current_vertex, axis_.axis) - getattr(vertex_new, axis_.axis)) > radius:
            # prune subtree
            pass
        else:
            vertices_near, radius = near_vertices2(vertex_new, next_vert2, current_vertex, k, vertices_near, axis_.next_, tree_num)
    return vertices_near, radius
##############################################################################################################
